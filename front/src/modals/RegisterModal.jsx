...
export default function RegisterModal({ isOpen, onClose, onOpenHostModal, onSuccess, onSwitchToLogin }) {
  ...
  async function registerEVOwner() {
    if (!name || !phone || !email || !password) { showToast('Please fill in all fields', 'error'); return; }
    if (!terms) { showToast('Please accept the Terms of Service', 'error'); return; }

    try {
      const { data } = await client.post('/api/register', { name, phone, email, password, type: 'ev-owner' });
        ...
        <div className="modal-body">
          ...
          {regType === 'ev-owner' && (
            <div>
              ...
              <button className="btn btn-primary btn-lg" style={{ width: '100%', marginBottom: '1rem' }} onClick={registerEVOwner}>
